...
Algorithm:
  1. For each test sample reconstruct bus measurements (V, P, Q) from the
     normalised features stored in full_dataset.csv.
  2. Run a linearised DC WLS state estimation on the pandapower case14 network.
  3. Compute the weighted residual norm J = r^T W r, where r = z - h(x_hat)
     and W = diag(1/sigma_i^2).
  4. Compare J to the chi-square threshold chi2(alpha, dof) to label the sample
     as "normal" or "attack".

Split: identical to train_model.py
    train_test_split(test_size=0.2, random_state=42, stratify=y)

Outputs:
    Console  : classification report + comparison table vs ML ensemble
    data/chi_square_results.json

Usage:
    python src/chi_square_baseline.py
"""
...
print(f"  Chi-square threshold : {chi2_threshold:.3f}  (alpha={ALPHA}, dof={dof})",
      flush=True)
...
# WLS bad-data test
# ---------------------------------------------------------------------------
# For a linearised DC model the WLS objective simplifies to the normalised
# residual norm J = sum_i  ((z_i - z_i_hat)^2 / sigma_i^2).
# We use the raw features as "measurements" z_i and estimate z_hat as the
# per-feature training-set mean (the expected value under normal operation).
# This is the simplest reproducible formulation that mirrors what classical
# SCADA BDD implementations do without requiring a running power flow.

print("\nComputing WLS residuals ...", flush=True)

# Training-set mean (serves as the "estimated state" h(x_hat))
...
X_train_normal = X_train_tmp.copy()          # intentional: use full train split
# (we use all training data; in practice only normal-labelled rows would be used
#  for the reference mean, but we keep it simple and consistent here)
...
], axis=1)                                   # shape: (n_test, n_meas)

residuals = X_test_meas - z_hat             # (n_test, n_meas)
J = (residuals**2 * weights).sum(axis=1)    # weighted residual norm, (n_test,)

# Classify: J > threshold  =>  bad data detected
y_pred_chi2 = (J > chi2_threshold).astype(int)

# Soft score for ROC-AUC: normalised J
J_norm = J / chi2_threshold                  # > 1 means flagged

# ---------------------------------------------------------------------------
# Metrics
# ---------------------------------------------------------------------------
print("\n" + "=" * 62, flush=True)
print("  CHI-SQUARE WLS BASELINE RESULTS", flush=True)
print("=" * 62, flush=True)
...
results = {
    ...
    "comparison_vs_ml": {
        ...
        "recall_lift_pp":  round((float(ml_recall) - chi_recall) * 100, 2)
